...
All binaries are uploaded to a single rolling release tag, so every script or wiki snippet that downloads from `/releases/latest/download/` or `/releases/download/rolling-release/` always targets the freshest build without changing URLs.
...
* Releases reuse the single `rolling-release` tag, avoiding multiple GitHub download URLs while still attaching the latest binaries.
